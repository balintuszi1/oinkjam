...
func _process(delta: float) -> void:
	if Global.active_elevator == self and is_player_inside:
		if Input.is_action_just_pressed("interact"):
			on_elevator_leave()
		if is_door_open: #only allow movement if the doors are opened
...
func on_elevator_leave(direction=null):
	...
	leave(Global.player, direction)

func show_arrows():
	ui.show()
	if Global.current_floor >= Global.MAX_FLOORS: up_arrow.hide()
	else: up_arrow.show()
	if Global.current_floor <= 1: down_arrow.hide()
	else: down_arrow.show()
